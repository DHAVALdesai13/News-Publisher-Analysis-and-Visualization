...
//#########################################################################################
draw("#my_dataviz",".radarChart","#my_dataviz_player","#myAreaChart4");

function draw(selectdiv1,selectdiv2,selectdiv3,selectdiv4){
  rich(selectdiv1);
  dhav(selectdiv2);
  dylan(selectdiv3);
  annie(selectdiv4);
  ...
  d3v3.select("#startYear").on("change", function(d) {
    // recover the option that has been chosen
  ...
  d3v3.select("#endYear").on("change", function(d) {
  // recover the option that has been chosen
  var selectedOption = d3.select("#selectButton").property("value")
  var selectedplayer = d3.select("#selectButton_player").property("value")
  // run the updateChart function with this selected option
...
function annie(selectdiv4){
  d3v3.csv("football_stats.csv", function(data) {
...
*/
    ...
    var div = d3v3.select("body").append("div").attr("class", "toolTip");

    var formatPercent = d3v3.format("");
    ...
    var svg = d3v3.select(selectdiv4).append("svg")
    ...
    function change(startYear, endYear) {
        ...
        //console.log(dataset)
        var dataFilter = data.filter(function(d){return d.Year >= startYear && d.Year <= endYear})
        console.log("annie filter",dataFilter)

        var numWins = d3v3.nest()
        .key(function(d) { return d.Tm; })
        .rollup(function(v) { return d3v3.sum(v, function(d) {return d.W}) })
        .entries(dataFilter);
       ...
       // console.log(d3.max(numWins).values)
        ...
        //data = dataFilter
        var x0 = dataFilter2.sort(function(a,b) {
        ...
        y.domain(x0);
        x.domain([0, d3v3.max(numWins).values + 6
        ]);
...
function dylan(selectdiv3){
      ...
      var margin = {top: 10, right: 150, bottom: 40, left: 100},
      ...
      d3.csv("QB_data.csv", function(data) {
        ...
        svg.append("text")
          ...
          var mousemove = function(d) {
              Tooltip
              Tooltip
              .html("Team: " + d.Tm + "<br>" + "TDs: " + d.TD + "<br>" + "Yards: " + d.Yds + "<br>" + "GWDs: " + d.GWD * 100 + "<br>" + "Ints: " + d.Int + "<br>" + "Cmp %: " + d.Cmpp + "%")
          ...
          function update(selectedPlayer, startYear, endYear, category) {
              ...
              xAxis.transition().duration(1000).call(d3.axisBottom(x).ticks(numTicks).tickFormat(d3.format("d")))
    ...
    var mousemove = function(d) {
        Tooltip
          .html("Wins: " + d.W + "<br>" + "Losses: " + d.L + "<br>" + "W/L %: " + d.WL * 100 + "%" + "<br>" + "Pts For: " + d.PF + "<br>" + "Pts Agst: " + d.PA)
          .style("left", (d3.mouse(this)[0]+70) + "px")
      ...
      d3.csv("football_stats.csv", function(data) {
          ...
          console.log("data rich" , allGroup)
          // add the options to the button
        ...
        svg.append("text")
         ...
         .on("mouseleave", mouseleave);
          ...
          function update(selectedGroup, startYear, endYear) {
            ...
            xAxis.transition().duration(1000).call(d3.axisBottom(x).ticks(numTicks - 1).tickFormat(d3.format("d")))
            // Give these new data to update line
            console.log(dataFilter)
            line
                .datum(dataFilter)
